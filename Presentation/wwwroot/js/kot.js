...
$(".ready-item-checkbox").change(function () {
    ...
    if ($(this).is(":checked")) {
        ...
        readyItems = readyItems.filter(item => item.Id !== parseInt(itemId) || !item.Modifiers.every(modifier => modifierIds.includes(modifier.toString())));   
    }
});
...
function markItemAsReady() {
    ...
    $.ajax({
        ...
        data: { pageIndex: pageIndexOfModal , readyItems: listToSend, orderId: orderIdOfModal, categoryId: currentCategoryId, inReady: inReady },
        success: function (data) {
            $("#kotCardsPartialStart").html(data);
...
function openMarkedAsPreparedModal(orderId) {
    $.ajax({
        ...
        data: { pageIndex: pageIndexOfModal2, orderId: orderId, categoryId: currentCategoryId, inReady: inReady },
        success: function (data) {
