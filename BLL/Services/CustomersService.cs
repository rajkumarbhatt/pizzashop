...
public class CustomersService : ICustomerService
{
    ...
    public CustomersService(PizzaShopContext context, ILogger<CustomersService> logger)
    {
        _context = context;
        _logger = logger;
    }
    public async Task<List<CustomerTable>> GetCustomersAsync()
    {
        try
        {
            ...
            foreach (Customer customer in customerList)
            {
                ...
                List<Order> orders = await _context.Orders
                    .Where(o => o.CustomerId == customer.Id && o.IsDeleted == false)
        ...
        {
            _logger.LogError(ex, "An error occurred while fetching customers: {Message}", ex.Message);
    ...
    public async Task<CustomerViewModel> GetCustomerDetailsTableAsync()
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while fetching customer details: {Message}", ex.Message);
            Console.WriteLine($"An error occurred: {ex.Message}");
            return new CustomerViewModel
            {
                Customers = new List<CustomerTable>()
            };
        }
    }
    public async Task<CustomerViewModel> FilterCustomersAsync(int pageIndex, int pageSize, string searchValue, string time, string sort, string order, string fromDate, string toDate)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while filtering customers: {Message}", ex.Message);
            Console.WriteLine($"An error occurred: {ex.Message}");
            return new CustomerViewModel
            {
                Customers = new List<CustomerTable>()
    ...
    public async Task<List<CustomerTable>> GetCustomersBasedOnFiltersAsync(string time, string searchValue, string fromDate, string toDate)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while fetching customers based on filters: {Message}", ex.Message);
            Console.WriteLine($"An error occurred: {ex.Message}");
            return new List<CustomerTable>();
        }
    }
    public async Task<CustomerHistory> GetCustomerDetailsAsync(int customerId)
    {
        try
        {
            Customer customer = await _context.Customers.FirstOrDefaultAsync(c => c.Id == customerId) ?? new Customer();
            List<Order> orders = await _context.Orders
    ...
    public async Task<byte[]> ExportCustomersAsync(string time, string searchValue, string fromDate, string toDate)
    {
        try
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                _logger.LogInformation("Exported customers successfully.");
        ...
        {
            _logger.LogError(ex, "An error occurred while exporting customers: {Message}", ex.Message);
            Console.WriteLine($"An error occurred while exporting customers: {ex.Message}");
            return null;
        }
    }
}