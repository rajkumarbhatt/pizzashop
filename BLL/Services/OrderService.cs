...
public class OrderService : IOrderService
{
    ...
    public OrderService(PizzaShopContext context, ILogger<OrderService> logger)
    {
        _logger = logger;
        _context = context;
    }
    ...
    public async Task<List<OrderTable>> GetOrdersBasedOnFiltersAsync(string status, string time, string searchValue)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while retrieving orders based on filters");
            Console.WriteLine(ex.Message);
    ...
    public async Task<byte[]> GenerateInvoiceAsync(int orderId)
    {
        try
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                _logger.LogInformation("Invoice generated successfully for order ID {OrderId}", orderId);
                return ms.ToArray();
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while generating invoice for order ID {OrderId}", orderId);
            Console.WriteLine(ex.Message);
            return null;
        }
    }
    public async Task<byte[]> ExportOrdersAsync(string status, string time, string searchValue, string fromDate, string toDate)
    {
        try
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                _logger.LogInformation("Excel file generated successfully for orders");
        ...
        {
            _logger.LogError(ex, "An error occurred while exporting orders to Excel");
            Console.WriteLine(ex.Message);
            return null;
        }
    }
    public async Task<string> EncryptOrderIdAsync(int orderId)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while encrypting order ID {OrderId}", orderId);
            Console.WriteLine(ex.Message);
            return "";
        }
    }
    public async Task<int> DecryptOrderIdAsync(string orderIdEncrypted)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred while decrypting order ID {OrderIdEncrypted}", orderIdEncrypted);
            Console.WriteLine(ex.Message);
            return 0;
        }
    }
