...
public class OrderAppService: IOrderAppService {
                ...
                if (waitingListModal.Id == -1) {
                    ...
                    if (await _context.WaitingLists.AnyAsync(w => w.Customer.Email == email && w.IsDeleted == false)) {
                        return new JsonResult(new {
                            success = false, message = "Customer already in waiting list"
                        });
                    }
                    ...
                    if (customer != null && await _context.Orders.AnyAsync(o => o.CustomerId == customer.Id && (o.Status == "Pending" || o.Status == "In Progress" || o.Status == "Served"))) {
                        return new JsonResult(new {
                            success = false, message = "Customer already has an ongoing order"
                        });
                    }
                    ...
                    if (waitingList == null) {
                        return new JsonResult(new {
                            success = false, message = "Waiting list not found"
                        });
                    }
                    ...
                    await transaction.CommitAsync();
                    return new JsonResult(new {
                        success = true, message = "Waiting list updated successfully"
                    });
                }
            } catch {
    ...
    public async Task < JsonResult > GetWaitingListForCurrentSectionAsync(int sectionId) {
        ...
        return new JsonResult(new {
            success = true, customerDetails = waitingListTables
        });
    }
            ...
            try {
                if (tableIds.Count == 1) {
                    ...
                    if (table.Capacity < waitingListModal.NumberOfPeople) {
                        return new JsonResult(new {
                            success = false, message = "Customers can't be managed in selected table"
                        });
                    }
                }
                ...
                foreach(int tableId in tableIds) {
                    ...
                    if (table.Status == "Assigned" || table.Status == "Running") {
                        return new JsonResult(new {
                            success = false, message = "The table is already assigned."
                        });
                    }
                    if (table.Capacity > waitingListModal.NumberOfPeople && tableIds.Count > 1) {
                        return new JsonResult(new {
                            success = false, message = "Customers can be managed in less than selected tables"
                        });
                    }
                }
                if (tableIds.Count >= 1) {
                    ...
                    return new JsonResult(new {
                        success = false, message = "You can assign " + optimumTable.Name + " for optimal arrangement"
                    });
                }
                assign:
                    int capacity = 0;
                ...
                if (capacity < waitingListModal.NumberOfPeople)
                    return new JsonResult(new {
                        success = false, message = "Customers can't be managed in selected tables"
                    });
                ...
                if (waitingListModal.Id == -1) {
                    if (await _context.WaitingLists.AnyAsync(w => w.Customer.Email == waitingListModal.Email && w.IsDeleted == false)) {
                        if ((await _context.WaitingLists.FirstOrDefaultAsync(w => w.Customer.Email == waitingListModal.Email && w.IsDeleted == false)).SectionId == waitingListModal.SectionId) {
                            return new JsonResult(new {
                                success = false, message = "Assign customer from waiting list"
                            });
                        } else {
                            return new JsonResult(new {
                                success = false, message = "Customer already in waiting list of another section"
                            });
                        }
                    }
                ...
                await transaction.CommitAsync();
                return new JsonResult(new {
                    success = true, message = "Tables assigned successfully", orderId = order.Id
                });
            } catch {
                await transaction.RollbackAsync();
                return new JsonResult(new {
